...
from nets.graph_encoder import GraphAttentionEncoder , MultiHeadEncoder,MultiHeadEncoderPos, MultiHeadEncoderDyn
...
class AttentionModel(nn.Module):
    ...
    def __init__(self,
        ...
        self.pos_encoder = mySequential(*(
                MultiHeadEncoderPos(self.n_heads,
                                self.embedding_dim,
                                self.hidden_dim,
                                self.normalization,
                                )
    ...
    def forward(self, input, return_pi=False):
        ...
        if self.checkpoint_encoder:
            ...
            h_pos.shape
            pos_embeddings = self.pos_encoder(h_pos, st_edge)[0]
            ...
            static_embeddings= self.static_encoder(s_embed, st_edge)[0]
            dyn_embeddings =(self.dyn_encoder(d_embed, st_edge)[0])[-1]
            dyn_embeddings.shape
            fusion_embeddings = torch.cat((pos_embeddings,static_embeddings, dyn_embeddings), dim=-1)

...
class MultiHeadPosCompat(nn.Module):
    ...
    def forward(self, pos, edge):
       # h should be (batch_size, graph_size, input_dim)
        ...
        V = torch.matmul(posflat, self.W_val).view(shp)

        Q.shape
        att=torch.matmul(Q, K.transpose(2, 3))
        att.shape

        compatibility=(att+edge)*self.norm_factor
...
class MultiHeadEncoderPos(nn.Module):
    ...
    def __init__(
        ...
        super(MultiHeadEncoderPos, self).__init__()

        self.MHA_sublayer = MultiHeadAttentionPossubLayer(
            n_heads,
            embed_dim,
            feed_forward_hidden,
        ...
        self.FFandNorm_sublayer = FFandNormPossubLayer(
            n_heads,
            embed_dim,
            feed_forward_hidden,
            normalization=normalization,
...
class MultiHeadAttentionPossubLayer(nn.Module):
    ...
    def __init__(
        ...
        super(MultiHeadAttentionPossubLayer, self).__init__()

        self.MHA = MultiHeadPosCompat(
            n_heads,
            input_dim=embed_dim,
            embed_dim=embed_dim
        )
...
class FFandNormPossubLayer(nn.Module):
    ...
    def __init__(
        ...
        super(FFandNormPossubLayer, self).__init__()
...
class MultiHeadAttentionNew(nn.Module):
    ...
    def forward(self, q, st_edge, h=None,): #d_q, out_source_attn,
        ...
        heads = torch.matmul(F.softmax(compatibility, dim=-1), V)
...
class MultiHeadAttentionNewDyn(nn.Module):
    ...
    def forward(self, q, st_edge, h=None, ):  # d_q, out_source_attn,
        ...
        # for i in range(graph_size):
        #     aa=Q[:,:,:,i,:][:,:,:,None,:]
        #     aa2=(edg[:,:,i][None, :,:,None]).expand(time, batch_size, graph_size, graph_size)
        #     aa2.shape
        #     bb=torch.matmul(aa2, K)
        #     bb.shape
        #     a=torch.matmul(aa, bb.transpose(3, 4))
        #     att.append(a)
        # compatibility=torch.stack(att,0).squeeze(4).permute(1, 2,3, 0, 4)
        # compatibility.shape #8,2,51,51

        st_edge.shape
        aa = torch.matmul(Q, K.transpose(3, 4))
        aa.shape
        # Calculate compatibility (n_heads, batch_size, n_query, graph_size)
        compatibility = self.norm_factor * (aa + st_edge)
...
class MultiHeadEncoderDyn(nn.Module):
    ...
    def forward(self, input1, input2):
        out1, out2= self.MHA_sublayer_dyn(input1, input2)
        return self.FFandNorm_sublayer_dyn(out1),out2
...
class MultiHeadAttentionsubLayerDyn(nn.Module):
    ...
    def forward(self, input1, input2):
        # Attention and Residual connection
        out1, out2  = self.MHA_dyn(input1, input2) #, out2
        # Normalization
        return self.Norm(out1 + input1), out2

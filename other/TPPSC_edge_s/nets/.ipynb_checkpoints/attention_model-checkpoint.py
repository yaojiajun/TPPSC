...
class AttentionModel(nn.Module):
    ...
    def __init__(self,
        ...
        if self.is_hcvrp:
            ...
            num_veh = workers_size
            node_dim = 4 #+ num_veh  # x,y, demand(5 vehicles)
            ...
            self.init_pay_depot = nn.Linear(1, embedding_dim)
        ...
        self.robots_state_query_embed = nn.Linear(3, embedding_dim)# 1
        self.robot_taking_decision_query = nn.Linear(3, embedding_dim)# 1
    ...
    def forward(self, input, return_pi=False):
        ...
        if self.checkpoint_encoder:
            ...
            s_embed, d_embed, edg = self._init_embed(input)
            d_embed.shape
            h_pos= self.position_encoding(input, self.embedding_dim)
            h_pos.shape
            pos_em = self.pos_encoder(h_pos, edg)
            ...
            static_embeddings= self.static_encoder(s_embed, pos_em, edg)[0]
            dyn_embeddings =(self.dyn_encoder(d_embed, edg)[0])[-1]
    ...
    def _init_embed(self, input):
        ...
        if self.is_hcvrp:
            ...
            time_edg1 = torch.mul(time_cond1, time_cond2)
            time_edg1.shape
            time_edg1[time_edg1 <= 0] = 1000
            time_edg1[time_edg1 != 1000] = -1000

            time_edg2 = torch.mul(time_cond1, time_cond4)
            time_edg2.shape
            time_edg2[time_edg2 <= 0] = 1000
            time_edg2[time_edg2 != 1000] = -1000
            t_edg = time_edg1 + time_edg2
            t_edg[t_edg >= 0] = 1
            t_edg[:, :, 0] = 1
            t_edg[:, 0, :] = 1
            time_matrix = torch.abs(time_cond1)
            t_edg.shape
            t_edg = t_edg * time_matrix
            t_edg = 1 - t_edg / t_edg.max()
            edg = t_edg + distance_matrix
            edg[edg < 0] = 0
            edg = edg / edg.max()
            edg[edg <= 0] = 0
    ...
    def select_veh(self, input, state, sequences, embeddings, obj, veh_list, tour):
        ...
        if sequences:
            ...
            veh_context = torch.cat(((tour_dis[:, :].unsqueeze(-1) / SPEED[:, :]), current_loc[:, :]), -1).view(
                [batch_size, -1])
        else:
            ...
            veh_context=torch.cat(((tour_dis[:, :].unsqueeze(-1) / SPEED[:, :]), current_loc[:, :]), -1).view([batch_size, -1])


        #veh_context[working_workers]=-math.inf
        # output_context=torch.nn.Linear(3*num_veh, self.embedding_dim).cuda()
        veh_context = self.FF_veh(veh_context)
        # output_tour = torch.nn.Linear(num_veh*self.embedding_dim, self.embedding_dim).cuda()
        tour_context = self.FF_tour(mean_tour)

        context = torch.cat((veh_context, tour_context), -1).view(batch_size, self.embedding_dim * 2)
        select_embed = torch.nn.Linear(self.embedding_dim * 2, num_veh).to(device=SPEED.device)

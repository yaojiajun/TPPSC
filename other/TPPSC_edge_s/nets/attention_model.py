...
class AttentionModel(nn.Module):
    ...
    def _init_embed(self, input):
        ...
        if self.is_tppsc:
            ...
            distance_matrix = (all_loc[:, :, None, :] - all_loc[:, None, :, :]).norm(p=2,
                                                                                                              dim=-1).to(
                device=t_pay.device)
            ...
            time_matrix[time_matrix>=6]=-1
            time_matrix=1-time_matrix/time_matrix.max()
            ...
            # t_edge = t_edge / t_edge.max()

            edge = distance_matrix+time_matrix
            edge[edge < 0] = 0
            edge = edge / edge.max()
            edge[edge <= 0] = 0

...
def generate_tpsc_data(size_t, size_w,num_samples):
    max_n_agent = size_w
    # data = []
    # t_loc=[]
    period = 6
    ...
    w_capacity = torch.randint(1, 25, (max_n_agent, 1), dtype=torch.float,
                               device=w_loc.device).view(-1)
    w_speed = ((torch.randint(6, 10, (max_n_agent, 1)).to(torch.float)) / 10)
    ...
    for i in range(num_samples):
        ...
        init_pay = torch.randint(1, 100, (size_t, 1)).to(torch.float).squeeze(-1)
        pay = []
        ...
        if size_t==50:
            save_dataset(data, datadir + "/" + "5-50my" +"test" + "_tasks_" + problem + ".pkl")
        if size_t == 80:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--name", type=str, default='tpsc', help="Name to identify dataset")
    ...
    dataset = generate_tpsc_data(opts.tasks_size,opts.workers_size, 1)

...
class AttentionModel(nn.Module):
    ...
    def __init__(self,
        ...
        self.temp = 1.0
        self.is_mtasc = problem.NAME == 'mtasc'
    ...
    def _init_embed(self, input):
        ...
        if self.is_mtasc:
            ...
            sti_emb = torch.cat(  # [batch_size, graph_size+1, embed_dim]
                (
            ...
            dyn_emb = self.init_dynamic_embed(t_pay).permute(1, 0, 2, 3)

            return sti_emb, dyn_emb, st_edge
    ...
    def select_veh(self, input, state, sequences, embeddings, obj, veh_list, tour):
        ...
        workers_speed=(state.speed[:, 0:num_veh, :].clone())
        w_score = (state.workers_score[:, 0:num_veh, :].clone())
        w_start = (state.workers_decision_time[:, 0:num_veh, :].clone())
        w_deadline = (state.workers_deadline_time[:, 0:num_veh, :].clone())
        ...
        # w_speed = workers_speed[:, :, :] / workers_speed[:, :, :].max()
        ...
        if sequences:
            ...
            veh_context = torch.cat(((tour_dis[:, :].unsqueeze(-1) / workers_speed[:, :]), current_loc[:, :], w_score[:, :], w_start[:, :], w_deadline[:, :]), -1).view(
                [batch_size, -1])
        else:
            ...
            veh_context=torch.cat(((tour_dis[:, :].unsqueeze(-1) / workers_speed[:, :]), current_loc[:, :], w_score[:, :], w_start[:, :], w_deadline[:, :]), -1).view([batch_size, -1])

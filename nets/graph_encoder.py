...
class MultiHeadPosCompat(nn.Module):
    ...
    def forward(self, pos, edg):
       # h should be (batch_size, graph_size, input_dim)
        ...
        aa=torch.matmul(Q, K.transpose(2, 3))
        aa.shape
        compatibility=(aa+edg)*self.norm_factor
        # att = []
...
class MultiHeadAttentionNew(nn.Module):
    ...
    def forward(self, q, out_source_attn, edg, h=None,): #d_q, out_source_attn,
        ...
        compatibility=(torch.matmul(Q, K.transpose(2, 3))+edg)*self.norm_factor
        ...
        return out, out_source_attn , edg
...
class MultiHeadAttentionNewDyn(nn.Module):
    ...
    def forward(self, q, edg, h=None,): #d_q, out_source_attn,
        # h should be (batch_size, graph_size, input_dim)
        if h is None:
            h = q  # compute self-attention
        ...
        aa.shape
        # Calculate compatibility (n_heads, batch_size, n_query, graph_size)
        compatibility = self.norm_factor * (aa + edg)

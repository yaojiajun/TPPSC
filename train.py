...
def train_epoch(model, optimizer, baseline, lr_scheduler, epoch, val_dataset, problem, tb_logger, opts):
    ...
    training_dataset = baseline.wrap_dataset(problem.make_dataset(
        size_t=opts.tasks_size,size_w=opts.workers_size, num_samples=opts.epoch_size, filename=opts.val_dataset,
        distribution=opts.data_distribution))  # data, baseline (cost of data)
    training_dataloader = DataLoader(training_dataset, batch_size=opts.batch_size, num_workers=1)

    # Put model in train mode!
    model.train()
    set_decode_type(model, "sampling") #sampling or greedy
    ...
    aa=torch.std(cost) / math.sqrt(len(cost))
...
def train_batch(
    ...
    # Calculate loss
    reinforce_loss =-1.0*((cost - bl_val) * (log_likelihood + log_veh)).mean()

...
def generate_mrta_data():
    with open('data/sc/didi_data_100.txt', encoding='utf-8') as file:
        data = []
        agents_starttime=[]
        agents_location=[]
        agents_deadline=[]
        ...
        for line in file:
            ...
            if data_line[2]=='w':
                ...
                agents_riato.append([float(data_line[9])])
            else:
        ...
        for i in range(count_arrived.size(0)):
            ...
            for j in range(finnal_time):
                if j<=i-1:
                    ...
                    for k in range(cur_task.size(0)):
                        pay1[j].append( float(cur_task[k]-0.1*cur_task[k]*(j-i-2)))
                elif i + 5 < j:
                    for k in range(cur_task.size(0)):
                        pay1[j].append(0)
                # pay.append(pay1)
        ...
        case_info = {
            ...
            'w_speed': torch.tensor(agents_speed, dtype=torch.float),
...
if __name__ == "__main__":
    ...
    save_dataset(dataset, datadir + "/" + "didimytest" + "_tasks_" + problem + ".pkl")
